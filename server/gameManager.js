/* ============================================================
   gameManager.js — delegates game actions to validation (shared coins)
   ============================================================ */
'use strict';
const Validation = require('./validation');

function getShared(room, player){
  if(room.sharedCoins!==undefined) return room.sharedCoins;
  return player.coins;
}
function setShared(room, value){
  room.sharedCoins = Math.max(0, Math.round(value));
  for(const p of room.players) p.coins = room.sharedCoins;
}
...
const GameManager = {
  playSlots(room, player, data){
    ...
    // use shared coins for validation
    const shared=getShared(room, player);
    const v=Validation.validateBet({...player, coins: shared, day: room.day}, 'slots', bet, cfg);
    if(!v.ok) return {ok:false, reason: v.reason};
    if(bet > shared) return {ok:false, reason:'НЕ ХВАТАЕТ МОНЕТ'};
    setShared(room, shared - bet);
    ...
    if(payout>0) setShared(room, getShared(room, player) + payout);
    ...
    return {
      ...
      coins: getShared(room, player), sharedCoins: getShared(room, player), dayEarnings: player.dayEarnings, teamEarnings: room.teamEarnings,
      time: room.time
    };
  },
  ...
  playRoulette(room, player, data){
    ...
    const shared=getShared(room, player);
    const v=Validation.validateBet({...player, coins: shared, day: room.day}, 'roulette', total, {});
    if(!v.ok) return {ok:false, reason: v.reason};
    if(total > shared) return {ok:false, reason:'НЕ ХВАТАЕТ МОНЕТ'};
    setShared(room, shared - total);
    ...
    return {ok:true, game:'roulette', bet:total, payout, net, bonus, safety, result:res.n, wins:res.wins, coins:getShared(room, player), sharedCoins:getShared(room,player), dayEarnings:player.dayEarnings, teamEarnings:room.teamEarnings, time:room.time};
  },
  ...
  playDice(room, player, data){
    ...
    const shared=getShared(room, player);
    const v=Validation.validateBet({...player, coins: shared, day: room.day}, 'dice', bet, {});
    if(!v.ok) return {ok:false, reason:v.reason};
    if(bet>shared) return {ok:false, reason:'НЕ ХВАТАЕТ МОНЕТ'};
    setShared(room, shared-bet);
    ...
    room.addTime(cost);
    if(room.phase==='casino' && room.teamEarnings>=room.quota) setTimeout(()=>room.checkDayEnd('quota'),800);
    return {ok:true, game:'dice', bet, payout, net, bonus, safety, a:res.a, b:res.b, sum:res.sum, mult:res.mult, won:res.won, coins:getShared(room, player), sharedCoins:getShared(room,player), dayEarnings:player.dayEarnings, teamEarnings:room.teamEarnings, time:room.time};
  },
  ...
  playCoinflip(room, player, data){
    ...
    const luck=player.items.includes('lucky_charm')?0.05:0;
    const shared=getShared(room, player);
    const v=Validation.validateBet({...player, coins: shared, day: room.day}, 'coinflip', bet, {});
    if(!v.ok) return {ok:false, reason:v.reason};
    if(bet>shared) return {ok:false, reason:'НЕ ХВАТАЕТ МОНЕТ'};
    setShared(room, shared-bet);
    ...
    room.addTime(cost);
    if(room.phase==='casino' && room.teamEarnings>=room.quota) setTimeout(()=>room.checkDayEnd('quota'),800);
    return {ok:true, game:'coinflip', bet, payout, net, bonus, safety, result:res.result, won:res.won, coins:getShared(room, player), sharedCoins:getShared(room,player), dayEarnings:player.dayEarnings, teamEarnings:room.teamEarnings, time:room.time};
  },

  playBlackjack(room, player, data){
    const bet=Math.floor(Number(data&&data.bet)||0);
    const shared=getShared(room, player);
    const v=Validation.validateBet({...player, coins: shared, day: room.day}, 'blackjack', bet, {});
    if(!v.ok) return {ok:false, reason:v.reason};
    if(bet>shared) return {ok:false, reason:'НЕ ХВАТАЕТ МОНЕТ'};
    setShared(room, shared - bet);
    ...
    return {ok:true, game:'blackjack', bet, payout, net, bonus, safety, outcome, playerHand, dealerHand, coins:getShared(room,player), sharedCoins:getShared(room,player), dayEarnings:player.dayEarnings, teamEarnings:room.teamEarnings, time:room.time};
  }
};
